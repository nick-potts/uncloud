...
// RemoveMachine removes a machine from the cluster by its ID.
func RemoveMachine(ctx context.Context, cli *client.Client, id string) error {
	_, err := cli.ClusterClient.RemoveMachine(ctx, &pb.RemoveMachineRequest{Id: id})
	return err
}
